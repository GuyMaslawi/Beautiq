...
## ביטול מנוי לא עוצר את הכסף בלי `MAKE_GROW_CANCEL_WEBHOOK_URL`

כשבעלת עסק מבטלת, Allura סוגרת את הגישה בסוף התקופה — אבל הוראת הקבע ב-Grow
ממשיכה לגבות מהכרטיס שלה. `isDirectDebitCancelConfigured()` נכתב בדיוק בשביל
להזהיר על כך ולא נקרא משום מקום. עכשיו: כל ביטול שבו עצירת הוראת הקבע לא הצליחה
שולח התראה במייל עם `directDebitId` לעצירה ידנית, ו-`/admin/ops` מציג אזהרה
קבועה כל עוד המשתנה לא מוגדר.
