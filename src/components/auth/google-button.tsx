...
function GoogleSubmit({ label }: { label: string }) {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="border-border bg-card text-foreground hover:bg-muted/10 focus-visible:ring-primary/40 inline-flex h-11 w-full items-center justify-center gap-3 rounded-xl border font-medium shadow-sm transition-colors focus-visible:ring-2 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60"
    >
      <GoogleIcon />
      <span>{label}</span>
    </button>
...
export function GoogleButton({ mode }: { mode: "login" | "signup" }) {
  const label =
    mode === "signup" ? AUTH.social.googleSignup : AUTH.social.googleLogin;
