...
export function checkEnv(): EnvCheckResult {
  ...
  if (isProd) {
    ...
    // שחזור סיסמה עצמאי נשען כולו על אימייל: בלי ספק דואר מוגדר, בעלת עסק
    // שסיסמתה דלפה או נשכחה תקועה עד שמנהל יאפס אותה ידנית — בדיוק הפער
    // שהזרימה הזו נועדה לסגור.
    if (!emailChannel) {
      warnings.push(
        "RESEND_API_KEY / EMAIL_FROM אינם מוגדרים — שחזור סיסמה (\"שכחתי סיסמה\") לא " +
          "יעבוד: הקישור לא יישלח לאף אחת. בעלת עסק שתאבד גישה תזדקק לאיפוס ידני מהאדמין.",
      );
    }
    ...
    // אותו — השם מוצג בתחתית העמודים הציבוריים ובמסמכים המשפטיים.
    if (!isSet("NEXT_PUBLIC_LEGAL_ENTITY_NAME")) {
      warnings.push(
        "NEXT_PUBLIC_LEGAL_ENTITY_NAME לא מוגדר — העמודים הציבוריים והמסמכים המשפטיים " +
          "לא מציגים את שם הישות המשפטית שמפעילה את השירות (עוסק/חברה).",
      );
    }
