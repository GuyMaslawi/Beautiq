...
function PlanCard({ plan, featured, onSelect }: { plan: PlanInfo; featured: boolean; onSelect: () => void }) {
  if (featured) {
    return (
      <motion.div
        ...
        className="relative flex flex-col overflow-hidden rounded-[2rem] p-8"
        style={{
      ...
      >
        <div className="pointer-events-none absolute -top-16 -left-16 h-56 w-56 rounded-full opacity-30" style={{ background: "radial-gradient(circle, rgba(212,168,83,0.55) 0%, transparent 70%)" }} />

        <div className="absolute -top-3.5 right-8 flex items-center gap-1.5 rounded-full px-4 py-1 text-xs font-bold" style={{ background: "linear-gradient(135deg, #e5bd6a 0%, #c09560 100%)", color: "#3a2200", boxShadow: "0 4px 16px rgba(212,168,83,0.50)" }}>
        ...
        <div className="relative">
          <div className="mb-1.5 flex items-center gap-2.5">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: "rgba(212,168,83,0.18)", border: "1px solid rgba(212,168,83,0.38)" }}>
              <Gem className="h-5 w-5" style={{ color: "#e5bd6a" }} />
            </span>
            <h3 className="font-display text-2xl font-semibold text-white">{plan.name}</h3>
          </div>
          <p className="mb-6 text-sm leading-6" style={{ color: "rgba(255,255,255,0.62)" }}>{plan.tagline}</p>

          <div className="mb-6 flex items-baseline gap-1.5">
            <span className="font-display text-6xl font-bold tabular-nums" style={{ color: "#e5bd6a" }}>₪{plan.price}</span>
            <span className="text-sm" style={{ color: "rgba(255,255,255,0.50)" }}>/ לחודש</span>
          </div>

          <button
            onClick={onSelect}
            className="mb-7 flex w-full items-center justify-center gap-2 rounded-2xl py-3.5 text-base font-bold transition-transform duration-200 hover:-translate-y-0.5"
          ...
          {plan.featuresIntro && (
            <p className="mb-3 text-xs font-semibold" style={{ color: "rgba(229,189,106,0.85)" }}>{plan.featuresIntro}</p>
          )}
          <ul className="flex flex-col gap-2.5">
            {plan.features.map((f) => (
              <li key={f} className="flex items-center gap-2.5">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full" style={{ background: "rgba(212,168,83,0.18)", border: "1px solid rgba(212,168,83,0.35)" }}>
                  <Check className="h-3 w-3" style={{ color: "#e5bd6a" }} />
                </span>
                <span className="text-sm leading-5" style={{ color: "rgba(255,255,255,0.88)" }}>{f}</span>
              </li>
            ))}
          </ul>
        </div>
  ...
  return (
    ...
    >
      <div className="mb-1.5 flex items-center gap-2.5">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: "rgba(172,92,127,0.12)", border: "1px solid rgba(172,92,127,0.22)" }}>
          <Flower2 className="h-5 w-5" style={{ color: "var(--primary)" }} />
        </span>
        <h3 className="font-display text-2xl font-semibold" style={{ color: "var(--foreground)" }}>{plan.name}</h3>
      </div>
      <p className="mb-6 text-sm leading-6" style={{ color: "var(--muted)" }}>{plan.tagline}</p>

      <div className="mb-6 flex items-baseline gap-1.5">
        <span className="font-display text-6xl font-bold tabular-nums" style={{ color: "var(--primary)" }}>₪{plan.price}</span>
      ...
      <ul className="flex flex-col gap-2.5">
        {plan.features.map((f) => (
          <li key={f} className="flex items-center gap-2.5">
            <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full" style={{ background: "rgba(172,92,127,0.12)", border: "1px solid rgba(172,92,127,0.22)" }}>
              <Check className="h-3 w-3" style={{ color: "var(--primary)" }} />
            </span>
            <span className="text-sm leading-5" style={{ color: "var(--foreground)" }}>{f}</span>
          </li>
        ))}
      </ul>
    </motion.div>
...
function PlanSelection({ userName, onSelect }: { userName: string | null; onSelect: (id: PlanId) => void }) {
  return (
    <div className="relative mx-auto w-full max-w-5xl px-5 py-12 sm:py-16">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: EASE }} className="mb-12 text-center">
        <div className="mx-auto mb-6 inline-flex items-center gap-2 rounded-full px-4 py-1.5" style={{ background: "rgba(199,111,147,0.12)", border: "1px solid rgba(199,111,147,0.26)" }}>
        ...
        <p className="mx-auto mt-4 max-w-lg text-base leading-7" style={{ color: "rgba(255,255,255,0.60)" }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-10 flex flex-wrap items-center justify-center gap-x-6 gap-y-3">
        {[
